    ...
    def loop_forever(self):
        while True:
            ...
            if f'{a_host}.sock' not in os.listdir(self.SOCK_DIR):
                logging.info(f'{a_host}:{a_port} >con (new)')

                sock = socket.socket(socket.AF_UNIX)
                sock.bind(f'{self.SOCK_DIR}{a_host}.sock')
                sock.listen(1)
            ...
            d = threading.Thread(target=self.proxy_thread, args=(s_src, (a_host, a_port)))
            d.setDaemon(True)
            d.start()

    def proxy_thread(self, s_src, a_src):
        s_dst = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
        s_dst.connect(f'{self.SOCK_DIR}{a_src[0]}.sock')
...
server = Server('redpot:latest', bind_port=6379)
server.loop_forever()
